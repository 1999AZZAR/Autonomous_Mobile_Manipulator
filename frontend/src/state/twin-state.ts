...
const defaultState: TwinState = {
  ...
  gripperOpen: true,
  tiltAngle: 45,   // 45° = arm tilted forward 45°, clearly visible arm angle
  lifterHeight: 50,
  connected: false,
  mode: 'idle',
};
...
export function updateFromSensorData(data: SensorData) {
  state = {
    ...
    sensors: {
      ...
      line_left: data.line_left,
      line_center: data.line_center,
      line_right: data.line_right,
      tf_luna_distance: data.tf_luna_distance,
    },
